package com.laisontech.laisonloratools.location.utils;
...
public class LocationUtil {
    ...
    public static LatLng transformFromWGSToGCJ(LatLng wgLoc) {
        ...
        //如果使用的是百度定位，则直接显示
        if (DiffLocationService.LOCATION_TYPE_BAIDU == DiffLocationService.LocationType) {
            return new LatLng(wgLoc.latitude, wgLoc.longitude);
        }
    ...
    //判断两个点是否超过了距离
    public static boolean outOfRange(LatLng fromLatLng, LatLng toLatLng, int distance) {
        if (fromLatLng == null || toLatLng == null) return false;
        double distanceBetween = SphericalUtil.computeDistanceBetween(fromLatLng, toLatLng);
